...
extension TableState {
    ...
    var format: EmptyStateFormat {
        ...
        case .noInternet:
            ...
            var format = EmptyStateFormat()
            format.buttonColor = #colorLiteral(red: 0.1960784346, green: 0.3411764801, blue: 0.1019607857, alpha: 1).BlueColor(Alpha: 1)
            format.position = EmptyStatePosition(view: .bottom, text: .left, image: .top)
            format.verticalMargin = 40
            format.horizontalMargin = 40
            format.imageSize = CGSize(width: 320, height: 200)
           ...
           // format.gradientColor = ("3854A5".hexColor, "2A1A6C".hexColor)
            format.titleAttributes = [.font: UIFont.TitleFont(), .foregroundColor: UIColor.black]
            format.descriptionAttributes = [.font: UIFont.SubTitleFont(), .foregroundColor: UIColor.black]
            return format
