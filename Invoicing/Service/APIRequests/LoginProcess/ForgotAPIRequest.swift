...
class ForgotAPIRequest: NSObject {
    
    static let shared = ForgotAPIRequest()
    
    func forgotPassword(requestParams : [String:Any], completion: @escaping (_ message : String?, _ status : Bool,_ Verification : Bool) -> Void) {
      
            AlamofireRequest.shared.PostBodyForRawData(urlString: "BaseURL".ForgotpasswordURL, parameters: requestParams, authToken: "", isLoader: true, loaderMessage: "") { (data, error) in
