...
class ConfirmEmailAPIRequest: NSObject {
    
    static let shared = ConfirmEmailAPIRequest()
    
    func ConfirmEmail(requestParams : [String:Any], completion: @escaping (_ message : String?, _ status : Bool,_ session : Bool) -> Void) {
        
        AlamofireRequest.shared.PostBodyForRawData(urlString: "BaseURL".ConfirmEmailURL, parameters: requestParams, authToken: "", isLoader: true, loaderMessage: "") { (data, error) in
