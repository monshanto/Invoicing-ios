...
class CreateTaxRequest: NSObject {
    
    static let shared = CreateTaxRequest()
    
    func CreateTax(requestParams : [String:Any], completion: @escaping (_ message : String?, _ status : Bool,_ session:Bool) -> Void) {
        AlamofireRequest.shared.postDataFor(urlString: "BaseURL".CreateTaxURL, parameters: requestParams, authToken: AppUser.accessToken, imageData: nil, isLoader: true, loaderMessage: "Creating") { (data, error) in
            if error == nil{
                print(data as Any)
