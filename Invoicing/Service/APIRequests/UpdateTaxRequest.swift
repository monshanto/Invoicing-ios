...
class UpdateTaxRequest: NSObject {
    
    static let shared = UpdateTaxRequest()
    
    func UpdateTax(requestParams : [String:Any], completion: @escaping (_ message : String?, _ status : Bool,_ session:Bool) -> Void) {
        AlamofireRequest.shared.postDataFor(urlString: "BaseURL".UpdateTaxURL, parameters: requestParams, authToken: AppUser.accessToken, imageData: nil, isLoader: true, loaderMessage: "update item") { (data, error) in
            if error == nil{
                print(data as Any)
