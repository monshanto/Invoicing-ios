...
class CreateCustomerAPIRequest: NSObject {
    
    static let shared = CreateCustomerAPIRequest()
    
    func CreateCustomer(requestParams : [String:Any], completion: @escaping (_ message : String?, _ status : Bool,_ session:Bool) -> Void) {
//        AlamofireRequest.shared.postDataFor(urlString: "BaseURL".CreateCustomerURL, parameters: requestParams, authToken: AppUser.accessToken, imageData: nil, isLoader: true, loaderMessage: "Creating") { (data, error) in
             ...
             AlamofireRequest.shared.PostBodyForRawData(urlString: "BaseURL".CreateCustomerURL, parameters: requestParams, authToken: AppUser.accessToken, isLoader: true, loaderMessage: "Creating") { (data, error) in
