...
class TaxDeleteAPIRequest: NSObject {
    
    static let shared = TaxDeleteAPIRequest()
    
    func TaxDelete(requestParams : [String:Any], completion: @escaping (_ message : String?, _ status : Bool,_ session:Bool) -> Void) {
        AlamofireRequest.shared.DeleteDataFor(urlString: "BaseURL".DeleteTaxURL, parameters: requestParams, authToken: AppUser.accessToken, imageData: nil, isLoader: true, loaderMessage: "Delete") { (data, error) in
            if error == nil{
                print(data as Any)
